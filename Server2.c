...
void loop(int sock)
{
	...
	while(k < BUFFERSIZE)
	{
		...
		int serverSock = socketForServer();
		...
		if(acceptWorker > 0)
		{
			...
			while(1)
			{
				iret1 = pthread_create(&read_thread, NULL, readThread, (void *) &serverSock);
			}
			
		}	
		
		if(acceptWorker == 0)			//child
		{
			int iret;
			...
			while(1)
			{
				...
				iret = pthread_create(&write_thread, NULL, writeThread, (void *) sock);
...
int socketForServer()
{
	...
	write( STDOUT_FILENO, sockbuff, strlen(sockbuff));
	listen(sock, BUFFERSIZE);
	int serverSock = accept(sock, NULL, NULL);
	return serverSock;
}
		...
		else 					//kill by name
		{
			for(j = 0; j < i; j++) 
			{
				...
				//if(token == null
					...
					{
						error  = write(STDOUT_FILENO, token, sizeof(token));
						if (error == -1 )
						{
							perror("Error in write1");
						}
